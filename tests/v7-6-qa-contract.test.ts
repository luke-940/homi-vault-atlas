...
describe("Atlas v7.6 resource-safe browser QA contract", () => {
  ...
  it("binds every route to explicit rendered-selector applicability instead of a non-empty aggregate", () => {
    ...
    expect(CORE_ROUTE_CASES[0].geometryRequiredSelectors)
      .toContain(".home-v75-graph-shell .semantic-space-host, .living-graph-canvas.is-home");
    expect(CORE_ROUTE_CASES[0].geometryRequiredSelectors).toContain(".home-v75-boundary");
    expect(CORE_ROUTE_CASES[0].geometryRequiredSelectors).not.toContain(".home-v75-evidence");
    expect(CORE_ROUTE_CASES[1].geometryRequiredSelectors).toContain(".home-v75-evidence");
  ...
  it("keeps the resource harness server-free and delegates pixel goldens to the CI-only Playwright suite", async () => {
    ...
    expect(source).toContain("workspaceMainLang: document.querySelector(runtimeSelectors.workspaceMain)?.getAttribute(\"lang\")");
    expect(source).toContain("return page.locator(aliasRuntimeSelector(selector))");
    expect(source).toContain("selectors: groupSelectors.map(aliasRuntimeSelector)");
    expect(source).toContain("requiredSelectors: route.geometryRequiredSelectors.map(aliasRuntimeSelector)");
    ...
    expect(source).toContain("document.querySelector(runtimeSelectors.homeHeadline)");
    expect(source).toContain("document.querySelector(runtimeSelectors.homeTerrain)");
    expect(source).not.toContain(".home-view-v74");
    expect(source).toContain("`${runtimeSelectors.app} small`, `${runtimeSelectors.app} span`");
