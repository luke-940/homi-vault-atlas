...
function validatePacks(packs) {
  ...
  if (packs.publication?.schema !== "atlas.publication.v2"
    || packs.publication.profile !== buildProfile
    || packs.publication.blockers?.length) failures.push("publication");
  ...
  return { schema: "atlas.public_pack_shape_validation.v2", failures: 0, pass: true };
}
...
const shapeValidation = validatePacks(packs);
...
const assetManifest = {
  schema: `atlas.${buildProfile}_assets.v2`,
...
const outputReceipt = {
  schema: `atlas.${buildProfile}_build.v2`,
  profile: buildProfile,
  publicSnapshotDigest: packs.publication.publicSnapshotDigest,
